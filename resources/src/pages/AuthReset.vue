...
const rules = {
  email: [
    {required: true, message: 'Поле "Email" обязательно для заполнения', trigger: 'change'},
    {type: 'email', message: 'Поле "Email" должно быть валидным email', trigger: 'change'}
  ],
};
  ...
  await formEl.validate(valid => {
    ...
    resetPassword(form.email).then(response => {
      if (response.status !== 200) {
        ...
        ElMessageBox.alert('Новый пароль для входа был отправлен на указанный email', '', {
          showClose: false,
          callback: function () {
  ...
  <div class="pt-8 px-6 h-screen bg-slate-400 flex justify-center">
    <div class="form-container rounded-lg bg-neutral-600 w-full md:w-1/2 2xl:w-1/5 h-min py-8 px-6">
      ...
      <div class="flex flex-col mb-4">
        ...
        <p class="text-2xl text-white text-center font-bold mb-2">Сброс пароля</p>
      </div>
      ...
      <div class="bg-neutral-600 mt-4">
        <div class="flex justify-center sm:justify-around flex-col sm:flex-row">
          <router-link to="login" class="text-green-700 hover:text-green-500 text-center p-2">Авторизоваться</router-link>
